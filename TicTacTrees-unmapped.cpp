...
// this version removes mapping to see difference in total game states
//total leaf nodes: 255168 when unmapped.
...
std::vector<TicTacNode*> vXwins;
std::vector<TicTacNode*> vOwins;
std::vector<TicTacNode*> vDwins;
...
void BuildTicTacTree(TicTacNode* Head) {
	...
	case 'x':
		Head->xWins();
		vXwins.push_back(Head);
		return;

	case 'o':
		Head->oWins();
		vOwins.push_back(Head);
		return;
...
void PrintBoards(std::vector<TicTacNode*> vect, char input){
	...
	int tWins = vXwins.size()+vOwins.size()+vDwins.size();
	
	//this take too long when repeat boards aren't mapped. probably don't uncomment. 

	// for(auto boards : vect){
	// 	auto board = boards->boardState;
	// 	std::cout << 
	...
	std::cout << input << " has won " << vect.size() << " out of " << tWins << "games. \n";
	return;
}
...
int main() {
	...
	bool exitFlag = false;
	while(!exitFlag){
		std::cout << "who wins? ( x, o, d) enter other input to exit: ";
		std::cin >> input;

		switch (tolower(input))
		{
		case 'x':
			PrintBoards(vXwins, input);
			break;

		case 'o':
			PrintBoards(vOwins, input);
			break;

		case 'd':
			PrintBoards(vDwins, input);
			break;
